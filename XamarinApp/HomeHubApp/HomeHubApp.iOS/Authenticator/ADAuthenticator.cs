    ...
    public async Task<ADAuthenticationResult> SignIn()
        {
            ...
            try
            {
                ...
                result = await _authContext.AcquireTokenAsync(_homeHubResourceId, 
                                                              _clientId, 
                                                              _redirectUri, 
                                                              new PlatformParameters(viewController));
        ...
        public async Task<string> GetToken()
        {
            ...
            try
            {
                ...
                // if token cache count is zero, AcquireTokenSilentAsync throws null exception instead of AdalException
                // catch it and throw the exception
                if (_authContext.TokenCache.Count == 0)
                {
                ...
                // result = await _authContext.AcquireTokenAsync(_homeHubResourceId, _clientId, _redirectUri, new PlatformParameters(UIApplication.SharedApplication.KeyWindow.RootViewController));
